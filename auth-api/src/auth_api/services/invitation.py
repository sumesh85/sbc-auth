...
class Invitation:
    ...
    def create_invitation(invitation_info: dict, user, invitation_origin, **kwargs):  # pylint: disable=too-many-locals
        ...
        if org.access_type == AccessType.GOVM.value:
            ...
            default_login_option_based_on_accesstype = (
                LoginSource.BCSC.value if org.access_type == AccessType.REGULAR.value else LoginSource.BCEID.value
            )
            account_login_options = AccountLoginOptionsModel.find_active_by_org_id(org.id)
            mandatory_login_source = getattr(
                account_login_options, "login_source", default_login_option_based_on_accesstype
            )

            if membership_type == ADMIN and mandatory_login_source == LoginSource.BCEID.value:
    ...
    def update_invitation(self, user, invitation_origin):
        ...
        for membership in self._model.membership:
            ...
            check_auth(org_id=org_id, one_of_roles=(ADMIN, COORDINATOR, STAFF))
            if membership.membership_type_code == ADMIN and self._model.login_source == LoginSource.BCEID.value:
                token_email_query_params["affidavit"] = "true"
    ...
    def get_invitations_for_org(org_id, status=None, **kwargs):
        ...
        if current_user_membership is None or current_user_membership.status != Status.ACTIVE.value:
            return []
    ...
    def validate_token(token):
        ...
        try:
            invitation_id = serializer.loads(
                token, salt=CONFIG.EMAIL_SECURITY_PASSWORD_SALT, max_age=token_valid_for
            ).get("id")
    ...
    def accept_invitation(invitation_id, user: UserService, origin, add_membership: bool = True, **kwargs):
        ...
        if add_membership:
            for membership in invitation.membership:
                ...
                existing_membership = MembershipService.get_membership_for_org_and_user(
    ...
    def _get_status_based_on_org(
        org_model: OrgModel, login_source: str, membership_model: MembershipModel, verified: bool
    ) -> str:
        if org_model.access_type == AccessType.GOVM.value:
